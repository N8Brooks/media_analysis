...
function step1bHelper(word: string, r1: number): string {
  ...
  if (
    l0 === word.charCodeAt(word.length - 2) &&
    (l0 === 98 ||
      l0 === 100 || l0 === 102 ||
      l0 === 103 || l0 === 109 ||
      l0 === 110 || l0 === 112 ||
...
export function stem(word: string): string {
  ...
  // r1
  if (
    word.length > 4 && (word.startsWith("gener") || word.startsWith("arsen"))
  ) {
    r1 = 5;
  ...
  if (
    (word.length === 6 || word.length === 7) &&
    EXCEPTIONAL_FORMS_POST_1A.hasOwnProperty(word)
  ) {
    return word;
  }
  ...
  if (l === 108) { // l = 108
    ...
    if (word.length - 1 >= r2) {
      ...
      word.length - 1 >= r1 && !isEndsWithShortSyllable(word.slice(0, -1))
    ) {
      word = word.slice(0, -1);
    }
  }
